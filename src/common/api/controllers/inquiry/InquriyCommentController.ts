...
export class InquiryCommentController {
   ...
   *       회원은 세션 쿠키를 통해 자동 식별되며,
   *       비회원은 비밀번호를 함께 전달해야 합니다.
   *     tags:
   *       - Inquiry Comment (문의 댓글)
   *     security:
   *       - sessionCookie: []  # 세션 인증 (회원 전용 기능용)
   *     parameters:
   *       - name: inquiryId
   *         in: path
   *         required: true
  ...
  private async createdInquiryComment(request: Request, response: Response) {
    try {
      ...
      const result: DefaultResponse<number> = await this.inquiryCommentService.createInquiryComment(
        inquiryId,
   ...
   *         description: 서버 내부 오류
   */
  private async inquiryCommentSearchListWithPaging(request: Request, response: Response) {
    try {
  ...
  private async inquiryCommentUpdate(request: Request, response: Response) {
    try {
      ...
      const result: DefaultResponse<number> = await this.inquiryCommentService.inquiryCommentUpdate(
        request,
